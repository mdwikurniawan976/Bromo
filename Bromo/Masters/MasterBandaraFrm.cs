...
namespace Bromo.Masters
{
    public partial class MasterBandaraFrm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Fields Tidak Boleh Kosong", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information); return;
            }
            else if (!unik(namaTextBox.Text,currentid ))
            {
                MessageBox.Show("Nama Bandara Sudah Digunakan , Pilih Nama Bandara Yang Lain", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
            }
            else if (!iata(kodeIATATextBox.Text,currentid) || jumlah > 3)
            {
                MessageBox.Show("Kode Iata Sudah Digunakan, Pilih Kode Yang Lain", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;

            }
            else if( jumlah > 3)
            {
                MessageBox.Show("Kode Yang Di input Harus Berupa 3 Huruf", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            else
            {
